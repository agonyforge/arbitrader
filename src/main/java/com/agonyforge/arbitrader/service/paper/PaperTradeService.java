...
public class PaperTradeService extends BaseExchangeService<PaperExchange> implements TradeService {
    ...
    private BigDecimal getCounterDelta(LimitOrder order) {
        ...
        counterDelta = counterDelta.subtract(counterFee).setScale(DecimalConstants.USD_SCALE, RoundingMode.FLOOR);
        return counterDelta;
    }
    ...
    private BigDecimal getBaseDelta(LimitOrder order) {
        ...
        BigDecimal baseFee = exchangeService.getExchangeMetadata(exchange).getFeeComputation() == FeeComputation.SERVER ? BigDecimal.ZERO : order.getOriginalAmount().multiply(feePercentage).setScale(DecimalConstants.BTC_SCALE,RoundingMode.HALF_EVEN);
    ...
    private BigDecimal getCounterFee(LimitOrder order) {
        BigDecimal cumulativeCounterAmount = order.getAveragePrice() != null ? order.getOriginalAmount().multiply(order.getAveragePrice()) : order.getOriginalAmount().multiply(order.getLimitPrice());
        BigDecimal feePercentage = getFee(order);
        return exchangeService.getExchangeMetadata(exchange).getFeeComputation() == FeeComputation.SERVER ? cumulativeCounterAmount.multiply(feePercentage) : BigDecimal.ZERO;
    }
